...
function mapBackendAnalysisToUI(
  ...
  // Build cultural summary with clear sections
  const culturalParts = [];
  
  if (cuesFormatted.length > 0) {
    culturalParts.push("Cultural cues:\n" + cuesFormatted.map(c => `• ${c}`).join("\n"));
  }
  
  if (norms.length > 0) {
    culturalParts.push("Social norms:\n" + norms.map(n => `• ${n}`).join("\n"));
  }
      ...
      <div className="grid gap-4 md:grid-cols-3 md:items-start">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900/40">
            <h2 className="text-base font-semibold tracking-tight">Coaching cues</h2>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900/40">
            <h2 className="text-base font-semibold tracking-tight">Real-time cues</h2>
            <p className="mt-2 text-sm text-zinc-700 dark:text-zinc-200">
              In live mode, WhisprAI provides subtle cues for pronunciation and pacing.
            </p>
...
function DetailCard({ title, body, bullets }: { title: string; body: string; bullets: string[] }) {
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900/40">
      ...
      <p className="mt-2 whitespace-pre-line text-sm text-zinc-700 dark:text-zinc-200">{body}</p>
